document.onload = carregarMenu();
document.onload = listaMensagens();

let contaClickComentarios = 0;
  ...
  fetch("http://localhost:3000/mensagens")
    ...
    .then(function (data) {
      data.forEach((mensagem) => {
        criarDivForum(mensagem);
      });
    })
    .catch((error) => console.error("Erro:", error));
}

function criarDivForum(data) {
  ...
  divForum.innerHTML += `
    ...
    <div class="flex flex-col justify-between w-full px-5 ">
        <div  id='box-texto-${data.id}' class='w-[95%] '>
                ...
                <svg class='cursor-pointer' onclick="ClickUpdateMensagem(${
                  data.id
        ...
        <h1 class="tracking-wide font-semibold text-pretty text-lg first-letter:uppercase w-[95%] pb-2">${
          data.titulo
        }</h1>
        <p class="leading-7 text-justify font-normal text-pretty text-base  first-letter:uppercase  w-full">${
          data.conteudo
        }</p>
                    ...
                    <div class="flex justify-between items-center  border-t-2  mt-2 h-12" >
                            ...
                            <svg onclick='chamacriarResposta(${
                              data.id
